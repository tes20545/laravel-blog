            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h1 class="text-xl font-semibold text-gray-900">ผู้ใช้งาน</h1>
                        <p class="mt-2 text-sm text-gray-700">เพิ่มผู้ใช้งาน</p>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                        <a href="{{ route('user.create') }}"><button type="button" class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">เพิ่มผู้ใช้งาน</button></a>
                    </div>
                </div>
                            ...
                            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">อีเมลล์</th>
                                        <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">ตำแหน่ง</th>
                                    </tr>
                                    </thead>
                                    ...
                                    @foreach($data as $key => $b)
                                        <tr>
                                            ...
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $b->email }}</td>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $b->position }}</td>
                                            <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                <a href="{{ route('user.edit',$b->id) }}" class="text-indigo-600 hover:text-indigo-900">แก้ไข</a>
                                                <a href="javascript:void(0);"
                                                   id="deleteTopic_{{ $b->id }}"
                                                   class="ml-4 font-medium text-red-600 hover:underline deleteAss"
                                                   data-id="{{ route('user.delete', $b->id) }}"
                                                   data-topic="{{ $b->name }}"
                                                   title="DELETE">
                                                    ลบ
                                                </a>
